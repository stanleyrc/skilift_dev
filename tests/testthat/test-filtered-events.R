...
test_that("create_oncotable handles multiple samples correctly", {
  ...
  results <- suppressWarnings(create_oncotable(
    cohort = test_cohort,
  ...
  expect_equal(names(results[[1]]), c("pair", "status", "error", "result"))
  
  # Test first sample (should succeed)
  expect_equal(results[[1]]$pair, "397089")
  expect_equal(results[[1]]$status, "success")
  expect_true(is.na(results[[1]]$error))
  expect_true(is.data.table(results[[1]]$result))
  
  # Test second sample (should fail)
  expect_equal(results[[2]]$pair, "397090")
  expect_equal(results[[2]]$status, "failed")
  expect_true(!is.null(results[[2]]$error))
  expect_null(results[[2]]$result)
